...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ! I NOTICE THAT WORKING WITH DYNAMIC SYNONYMS AND ANTONYMS IS THE BEST APPROACH 
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
TTree2* insertBST2(TTree2 *root, const char *word, WordNode *synonyms, WordNode *antonyms) {
    ...
    //*else case word already exists will be handled soon! ;)
...
TStack2* createStack2() {
    TStack2 *stack = (TStack2*)malloc(sizeof(TStack2));
    stack->top = NULL;
    return stack;
}

void push2(TStack2 *stack, TTree2 *node) {
...
bool isStack2Empty(TStack2 *stack) {
...
void HighSizeBST(TTree2 *tr){
    ...
    printf("The hight of the BST is : %d\n ",HighBSTHelper(tr));
}
...
TTree2 *BTSMerge(TTree2 *tr1, TTree2 *tr2){
    ...
    TQueue *q1=StoreBSTinOrder(tr1);
    TQueue *q2=StoreBSTinOrder(tr2);
    TQueue *merged=createQueue();
    TTree2 *dummy;
    
    while (!isEmptyQueue(q1) && !isEmptyQueue(q2))
    {
        if (strcmp(q1->front->treeNode->word, q2->front->treeNode->word) <= 0)
        {
            enqueue(merged, q1->front->treeNode);
            dummy=dequeue(q1);
            
        }
        else{
            enqueue(merged, q2->front->treeNode);
            dummy=dequeue(q2);
    ...
    int count = lenQueue(merged);
    if (count == 0) return NULL;  
    
    int mid = count/2;
    QNode** array = malloc(count * sizeof(QNode*));
    if (!array) return NULL;  
    
    QNode *current = merged->front;
    ...
    TTree2 *root = createTreeNode2(array[mid]->treeNode->word, array[mid]->treeNode->synonym, array[mid]->treeNode->antonym);
    
    //*build left subtree
    for (int i = 0; i < mid; i++)  //* handle all left nodes
    {
        root = insertBST2(root, array[i]->treeNode->word, array[i]->treeNode->synonym, array[i]->treeNode->antonym);
    }
